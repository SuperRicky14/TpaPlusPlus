...
public class TPToggle {
    public static void toggleTPOrWait(ServerPlayer executor) {
        if (Config.TOGGLE_WINDUP.get() == 0) {
            ...
            AsyncScheduler.schedule(new WindupData(Config.TOGGLE_WINDUP.get(), executor.getX(), executor.getY(), executor.getZ(), CommandType.TOGGLE, new ServerPlayer[]{executor}));
        }
    }
