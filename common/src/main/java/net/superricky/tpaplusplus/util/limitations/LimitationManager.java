...
import static net.superricky.tpaplusplus.util.limitations.LimitationType.*;
    ...
    public static Limitation getLimitationType(ServerPlayer sender, ServerPlayer receiver) {
        ...
        if (Boolean.FALSE.equals(sender.serverLevel().equals(receiver.serverLevel()))) {
            ...
            if (Boolean.FALSE.equals(Config.ALLOW_INTER_DIMENSIONAL_TELEPORT.get()))
                return new Limitation(DIFFERENT_DIMENSIONS, sender.serverLevel(), receiver.serverLevel());

            // If the configuration disables range checks interdimensional
            if (Boolean.TRUE.equals(Config.DISABLE_RANGE_CHECKS_INTER_DIMENSIONAL.get()) && sender.serverLevel().equals(receiver.serverLevel())) {
                return null;
            }
        }
        ...
        double distanceBetweenPlayers = TPAPlusPlus.euclideanDistanceCalculator3D(senderX, senderY, senderZ, receiverX, receiverY, receiverZ);
        ...
        if (Config.FURTHEST_ALLOWED_DISTANCE.get() != 0 && distanceBetweenPlayers > Config.FURTHEST_ALLOWED_DISTANCE.get())
            return new Limitation(TOO_FAR, distanceBetweenPlayers);
        ...
        if (Config.CLOSEST_ALLOWED_DISTANCE.get() != 0 && distanceBetweenPlayers < Config.CLOSEST_ALLOWED_DISTANCE.get())
            return new Limitation(TOO_CLOSE, distanceBetweenPlayers);
